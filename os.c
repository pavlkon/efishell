...
/* Input policy: only this translation unit assigns device/report meaning. USB HID 1.11 / HID Usage Tables: Generic Desktop X/Y/Wheel, Button, AC Pan. PS/2 packet identification and IntelliMouse negotiation are also OS policy.*/
...
static int os_hid_parse(const UINT8 *bytes, UINT32 size, os_hid_layout *out)
{
    ...
    for (UINT32 pos = 0; pos < size;) {
        ...
        if (prefix == 0xfe) { /* bounded long item; no standard input semantics */
...
static int os_ps2_mouse_init(void)
{
    ...
    if (!e)
        e = k_i8042_exchange(1, 0xf5, NULL, 0); /* disable streaming */
    if (!e)
        e = k_i8042_exchange(1, 0xf6, NULL, 0); /* defaults */
    if (!e)
        e = os_ps2_rate(200);
    if (!e)
        e = os_ps2_rate(100);
...
static void os_input_init(void)
{
    ...
    /* Discard AUX bytes captured before the negotiated packet format began; retain primary-port keystrokes that arrived during initialization. */
    k_input_report old;
...
/* Target-only decoder regression checks; never synthesize live input events. */
...
/* Target-side diagnostics for the reported RAM cache/scrolling regression. */
...
/*  live allocator + address-space + namespace tests */
...
/* live tasks + preemption + scheduling + synchronization */
static struct {
...
/* movq constant -> XMM0; sleep eight times (other contexts load their own FPU state); compare XMM0 after resumption; exit 0 only if it survived. */
...
/* File OPEN/WRITE/CLOSE from actual ring 3; target stays in the RAM @user root. */
...
/* disks and RAM-disk write fence  */
...
static void cmd_writetest(const char *directory)
{
    ...
    con_print(path);
    con_print(". Physical FAT persists across reboot; RAM does not.\n");
}
...
static void cmd_exec(const char *path)
{
    ...
    if (!f.size || f.size > 4 * 1024 * 1024 || (f.attributes & 0x10) || f.kind == 3) {
        error_("application size/type", K_EINVAL);
        return;
    }
    UINT32 slot;
    for (slot = 0; slot < 24 && launched[slot]; ++slot)
        ;
    if (slot == 24) {
        con_print("Use reap to release exited applications.\n");
        return;
    }
    UINT32 pages = (UINT32)((f.size + 4095) / 4096);
    UINT64 address = k_pmm_alloc_pages(pages, 0);
    if (!address) {
        error_("application buffer", K_ENOMEM);
        return;
    }
    UINT64 got = 0;
    e = k_vfs_read(&f, 0, (void *)(UINTN)address, f.size, &got);
    k_process *p = NULL;
    UINT32 id = 0;
    if (!e && got == f.size) {
        p = k_process_create(resolved);
        e = p ? k_rief_load(p, (void *)(UINTN)address, got, NULL, 0) : K_ENOMEM;
    } else if (!e)
        e = K_EIO;
    k_pmm_free_pages(address, pages);
    if (!e)
        e = k_process_start(p, &id);
    if (e) {
        if (p)
            k_process_destroy(p);
        error_("exec", e);
    ...
    launched[slot] = p;
    con_print("Started process ");
    con_print_uint(k_process_id(p));
    con_print(", task ");
    con_print_uint(id);
    con_print("\n");
}
static void cmd_reap(void)
{
    for (UINT32 i = 0; i < 24; ++i)
        if (launched[i]) {
            k_process_info p;
            if (!process_status(launched[i], &p) || p.state != K_TASK_ZOMBIE)
                continue;
            con_print("process ");
            con_print_uint(p.id);
            con_print(" exit=");
            ...
            con_print("\n");
            int e = k_process_destroy(launched[i]);
            if (!e)
                launched[i] = NULL;
            else
                error_("reap", e);
...
static void cmd_ps(void)
{
    ...
    con_print("states: 1 ready, 2 running, 3 sleeping, 4 blocked, 5 exited\n");
}
...
static void cmd_help(void)
{
    ...
    con_print("cp <source> <destination> | sync | fatwritetest (RAM only)\n");
    ...
    con_print("Run testall before smpstart, then smptest. User processes stay on CPU 0.\n");
}
...
/* boot handoff; no boot-service use after the first exit attempt */
static void boot_print(const CHAR16 *s)
{
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *system_table)
{
    ...
    kernel_usb_init();
    os_input_init();
    con_print("\nFAT16/32 file writes supported; raw physical writes blocked. RAM is volatile.\n");
