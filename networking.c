...
static void net_enqueue(UINT32 id, const void *p, UINT32 n)
{
...
static UINT32 nic_get(net_device *d, UINT32 reg) { return *(volatile UINT32 *)(d->regs + reg); }
static void nic_put(net_device *d, UINT32 reg, UINT32 value)
{
    *(volatile UINT32 *)(d->regs + reg) = value;
}
static void nic_fence(net_device *d)
{
    arch_write_fence();
    if (d->regs)
        (void)nic_get(d, 8);
}
static void nic_fail(net_device *d, int error)
{
    d->dead = TRUE;
    d->info.error = error;
...
static void nic_pci_prepare(UINT16 bdf, BOOLEAN io)
{
...
{
    nic_put(d, 0x20,
            ((UINT32)(write ? *value : 0)) | (reg << 16) | (1u << 21) |
                (write ? 1u << 26 : 1u << 27));
    if (!arch_wait_reg((void *)(d->regs + 0x20), 1u << 28, 1u << 28, 20))
        return K_ETIMEDOUT;
...
static int nic_intel_start(net_device *d, BOOLEAN advanced)
{
    ...
    if (!arch_wait_reg((void *)d->regs, 1u << 26, 0, 100))
        return K_ETIMEDOUT;
    k_delay_us(20000);
    nic_put(d, imc, 0xffffffff);
    if (advanced)
    ...
    if ((d->info.mac[0] & 1) || net_zero(d->info.mac, 6))
        return K_EIO;
    ...
    if (advanced) {
        ...
        if (!arch_wait_reg((void *)(d->regs + rb + 40), 1u << 25, 1u << 25, 100))
            return K_ETIMEDOUT;
        nic_put(d, 0xe14, nic_get(d, 0xe14) & ~0x20u);
    } else {
        nic_put(d, 0x2820, 0);
        nic_put(d, 0x3820, 0);
    ...
    ctrl |= 1u << 6;
    nic_put(d, 0, ctrl);
    if (advanced && nic_phy_claim(d, FALSE))
        return K_EBUSY;
    UINT16 phy;
    int phy_error = nic_mdio(d, 0, &phy, FALSE);
    if (!phy_error) {
        phy = (phy & ~(1u << 11)) | (1u << 12) | (1u << 9);
        phy_error = nic_mdio(d, 0, &phy, TRUE);
    }
    if (advanced) {
        int release = nic_phy_claim(d, TRUE);
        if (phy_error || release)
            return phy_error ? phy_error : release;
        nic_put(d, 0x18, nic_get(d, 0x18) | (1u << 28));
    }
    nic_put(d, 0x410, 10u | (8u << 10) | (6u << 20));
    nic_put(d, 0x400, 2u | 8u | (15u << 4) | (64u << 12));
...
static int nic_rtl_start(net_device *d)
{
...
static void nic_poll(net_device *d)
{
...
static int nic_transmit(net_device *d, const void *bytes, UINT32 n)
{
    ...
    for (UINT32 bus = 0; bus < 256; ++bus)
        for (UINT32 slot = 0; slot < 32; ++slot) {
            ...
            for (UINT32 function = 0; function < functions; ++function) {
                ...
                int result = K_ENOTSUP;
                str_copy(nd->info.driver, "unsupported", 16);
                if (platform.dma_allowed) {
                    if (vendor == 0x8086 &&
                        (device == 0x100e || device == 0x100f || device == 0x10d3 ||
                         device == 0x1010 || device == 0x107c)) {
                        str_copy(nd->info.driver, "intel-e1000", 16);
                        result = nic_intel_start(nd, FALSE);
                    } else if (vendor == 0x8086 &&
                               (device == 0x15f2 || device == 0x15f3 || device == 0x125b ||
                                device == 0x125c || device == 0x125d)) {
                        str_copy(nd->info.driver, "intel-i225/226", 16);
                        result = nic_intel_start(nd, TRUE);
                    } else if (vendor == 0x10ec && device == 0x8139) {
                        str_copy(nd->info.driver, "rtl8139", 16);
                        result = nic_rtl_start(nd);
                    }
                }
                if (result) {
                    nd->dead = TRUE;
