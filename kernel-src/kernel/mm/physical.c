...
UINT64 k_pmm_alloc_aligned(UINT32 count, UINT32 alignment, UINT64 maximum)
{
    if (!power2(alignment))
        return 0;
    if (!count)
...
UINT64 k_pmm_alloc_pages(UINT32 n, UINT64 max) { return k_pmm_alloc_aligned(n, 1, max); }
...
int k_as_alloc_ex(k_address_space *as, UINT64 bytes, UINT64 align, UINT32 rwx, UINT32 options,
                  UINT64 *base)
{
    return arch_as_alloc_ex(as, bytes, align, rwx, options, base);
}
int k_as_mapping_size(k_address_space *as, UINT64 address, UINT64 *bytes)
{
    return arch_as_mapping_size(as, address, bytes);
}
