...
int x86_64_timer_init(UINT32 hz)
{
    ...
    if (platform.apic) {
        ...
        if (!e && elapsed >= 100) {
            ...
            x86_64_timer_clock_init(hpet, hpet_period_fs, hpet_mask, lapic_period);
...
UINT64 x86_64_ticks(void) { return x86_64_clock_ticks(); }
UINT64 x86_64_uptime_ms(void) { return x86_64_clock_ms(); }
...
void x86_64_cpu_tables_init(cpu_state *cpu)
{
    ...
    cr4 = arch_read_cr4();
    /* Extended state is enabled after the common control-register setup. */
    cr4 &= ~((1ULL << 18) | (1ULL << 16) | (1ULL << 11) | (1ULL << 21));
    cr4 |= (1 << 9) | (1 << 10);
    if (platform.smep)
        cr4 |= 1 << 20;
    arch_write_cr4((UINT64)(UINTN)cr4);
    if (x86_64_xstate_enable())
        arch_stop();
...
void x86_64_wake_cpu(UINT32 cpu)
{
    if (!platform.apic || cpu >= platform.discovered_cpus || cpu == x86_64_cpu_id() ||
        !__atomic_load_n(&cpus[cpu].online, __ATOMIC_ACQUIRE))
        return;
    (void)send_ipi(cpus[cpu].apic_id, 0xf1);
}
