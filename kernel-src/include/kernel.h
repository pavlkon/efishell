...
/* Read-only driver snapshot; failure snapshots precede DMA shutdown. */
#define K_NET_DIAG_INTEL 1u
#define K_NET_DIAG_WAIT 2u
#define K_NET_DIAG_PHY_RESET_TIMEOUT 4u
#define K_NET_DIAG_PHY_VERIFIED 8u
typedef struct {
    char stage[32];
    UINT32 flags, revision, pci_command, control, status, eecd, mdic;
    UINT32 rx_control, tx_control, rx_desc_control, tx_desc_control;
    UINT32 rx_head, rx_tail, tx_head, tx_tail, rx_next, tx_next, tx_pending;
    UINT32 wait_register, wait_mask, wait_expected, wait_value;
    UINT32 phy_address, phy_id, phy_control, phy_status;
    UINT32 last_rx_status, last_rx_length;
    UINT64 rx_completed, tx_completed;
    INT32 error;
    UINT32 reserved;
    UINT32 phy_power, phy_reset_value;
} k_net_diag;
typedef struct {
...
int k_net_diagnose(UINT32, k_net_diag *);
